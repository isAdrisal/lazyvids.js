const lazyvids = (configObj => {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const log = (message, object) => {
      ...
      object ? window.console.log(`lazyvids: ${message}`, object) : window.console.log(`lazyvids: ${message}`);
    };

    const warn = (message, object) => {
      if (config.logLevel === 'silent') return;
      object ? window.console.warn(`lazyvids: ${message}`, object) : window.console.warn(`lazyvids: ${message}`);
    };

    const hasIo = typeof window.intersectionObserver === undefined ? false : true;
    let intersectionObserver; // Don't load videos on slow connections (optional)
    ...
    const mutationObserver = new MutationObserver(handleMutation); // Start observing for new lazyvids videos
